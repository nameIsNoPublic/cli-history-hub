...
| 后端 | server.js | `readCodexSessionHead()` - 读取前 256KB 提取 cwd/model/cli_version（带 mtime 缓存） |
...
- **按需解析**：`readCodexSessionHead()` 读取文件前 256KB（支持从 `turn_context` 获取实际模型名），结果按 mtime 缓存
- **空会话过滤**：`/api/projects` 和 `/api/projects/:pid/sessions-full` 均过滤 messageCount 为 0 的会话，确保计数一致
- **元数据缓存**：`extractCodexSessionMeta()` 使用 `sessionCache`（以 `codex:` 前缀区分），通过 JSONL 文件 mtime + sidecar mtime 联合失效

## 已知限制

- ~~Codex 会话不支持 sidecar 元数据~~ **已支持**：Codex 会话的 sidecar 存储在 `~/.codex/sessions/session-meta/{sessionId}.json`，支持重命名、标签、收藏
...
### Codex 会话的 sidecar 元数据（已实现）

Codex 会话已支持 sidecar 元数据：
- sidecar 存储在 `~/.codex/sessions/session-meta/{sessionId}.json`
- `extractCodexSessionMeta()` 读取 sidecar 的 `customName`/`tags`/`isFavorite`
- `PUT /meta` 路由通过 `isCodexProject()` 分支处理 Codex 写入
- 缓存失效逻辑已包含 sidecar mtime
